classdef (CaseInsensitiveProperties) ILines < DSS_MATLAB.Base
    ...
    methods (Access = public)
        ...
        function result = New(obj, Name)
            result = calllib(obj.libname, 'Lines_New', Name);
            obj.CheckForError();
        end
    ...
    methods
        ...
        function result = get.AllNames(obj)
            ...
            result = obj.apiutil.get_string_array('Lines_Get_AllNames');
        end

        function result = get.Count(obj)
            % Number of Line objects
            result = calllib(obj.libname, 'Lines_Get_Count');
        end

        function result = get.First(obj)
            % Set first object of Line; returns 0 if none.
            result = calllib(obj.libname, 'Lines_Get_First');
        end

        function result = get.Name(obj)
            % Get/sets the name of the current active Line
            result = calllib(obj.libname, 'Lines_Get_Name');
        end
        function obj = set.Name(obj, Value)
            calllib(obj.libname, 'Lines_Set_Name', Value);
            obj.CheckForError();
        end

        function result = get.Next(obj)
            % Sets next Line active; returns 0 if no more.
            result = calllib(obj.libname, 'Lines_Get_Next');
        end

        function result = get.idx(obj)
            % Get/set active Line by index;  1..Count
            result = calllib(obj.libname, 'Lines_Get_idx');
        end
        function obj = set.idx(obj, Value)
            calllib(obj.libname, 'Lines_Set_idx', Value);
            obj.CheckForError();
        end
        ...
        function result = get.Bus1(obj)
            ...
            result = calllib(obj.libname, 'Lines_Get_Bus1');
            obj.CheckForError();
        end
        function obj = set.Bus1(obj, Value)
            calllib(obj.libname, 'Lines_Set_Bus1', Value);
            obj.CheckForError();
        end

        function result = get.Bus2(obj)
            % Name of bus for terminal 2.
            result = calllib(obj.libname, 'Lines_Get_Bus2');
            obj.CheckForError();
        end
        function obj = set.Bus2(obj, Value)
            calllib(obj.libname, 'Lines_Set_Bus2', Value);
            obj.CheckForError();
        end

        function result = get.C0(obj)
            % Zero Sequence capacitance, nanofarads per unit length.
            result = calllib(obj.libname, 'Lines_Get_C0');
            obj.CheckForError();
        end
        function obj = set.C0(obj, Value)
            calllib(obj.libname, 'Lines_Set_C0', Value);
            obj.CheckForError();
        end

        function result = get.C1(obj)
            % Positive Sequence capacitance, nanofarads per unit length.
            result = calllib(obj.libname, 'Lines_Get_C1');
            obj.CheckForError();
        end
        function obj = set.C1(obj, Value)
            calllib(obj.libname, 'Lines_Set_C1', Value);
            obj.CheckForError();
        end

        function result = get.Cmatrix(obj)
            calllib(obj.libname, 'Lines_Get_Cmatrix_GR');
            obj.CheckForError();
            result = obj.apiutil.get_float64_gr_array();
        end
        function obj = set.Cmatrix(obj, Value)
            calllib(obj.libname, 'Lines_Set_Cmatrix', Value, numel(Value));
            obj.CheckForError();
        end

        function result = get.EmergAmps(obj)
            % Emergency (maximum) ampere rating of Line.
            result = calllib(obj.libname, 'Lines_Get_EmergAmps');
            obj.CheckForError();
        end
        function obj = set.EmergAmps(obj, Value)
            calllib(obj.libname, 'Lines_Set_EmergAmps', Value);
            obj.CheckForError();
        end

        function result = get.Geometry(obj)
            % Line geometry code
            result = calllib(obj.libname, 'Lines_Get_Geometry');
            obj.CheckForError();
        end
        function obj = set.Geometry(obj, Value)
            calllib(obj.libname, 'Lines_Set_Geometry', Value);
            obj.CheckForError();
        end

        function result = get.Length(obj)
            % Length of line section in units compatible with the LineCode definition.
            result = calllib(obj.libname, 'Lines_Get_Length');
            obj.CheckForError();
        end
        function obj = set.Length(obj, Value)
            calllib(obj.libname, 'Lines_Set_Length', Value);
            obj.CheckForError();
        end

        function result = get.LineCode(obj)
            % Name of LineCode object that defines the impedances.
            result = calllib(obj.libname, 'Lines_Get_LineCode');
            obj.CheckForError();
        end
        function obj = set.LineCode(obj, Value)
            calllib(obj.libname, 'Lines_Set_LineCode', Value);
            obj.CheckForError();
        end

        function result = get.NormAmps(obj)
            % Normal ampere rating of Line.
            result = calllib(obj.libname, 'Lines_Get_NormAmps');
            obj.CheckForError();
        end
        function obj = set.NormAmps(obj, Value)
            calllib(obj.libname, 'Lines_Set_NormAmps', Value);
            obj.CheckForError();
        end

        function result = get.NumCust(obj)
            % (read-only) Number of customers on this line section.
            result = calllib(obj.libname, 'Lines_Get_NumCust');
            obj.CheckForError();
        end
        ...
        function result = get.Parent(obj)
            ...
            result = calllib(obj.libname, 'Lines_Get_Parent');
            obj.CheckForError();
        end

        function result = get.Phases(obj)
            % Number of Phases, this Line element.
            result = calllib(obj.libname, 'Lines_Get_Phases');
            obj.CheckForError();
        end
        function obj = set.Phases(obj, Value)
            calllib(obj.libname, 'Lines_Set_Phases', Value);
            obj.CheckForError();
        end

        function result = get.R0(obj)
            % Zero Sequence resistance, ohms per unit length.
            result = calllib(obj.libname, 'Lines_Get_R0');
            obj.CheckForError();
        end
        function obj = set.R0(obj, Value)
            calllib(obj.libname, 'Lines_Set_R0', Value);
            obj.CheckForError();
        end

        function result = get.R1(obj)
            % Positive Sequence resistance, ohms per unit length.
            result = calllib(obj.libname, 'Lines_Get_R1');
            obj.CheckForError();
        end
        function obj = set.R1(obj, Value)
            calllib(obj.libname, 'Lines_Set_R1', Value);
            obj.CheckForError();
        end

        function result = get.Rg(obj)
            % Earth return resistance value used to compute line impedances at power frequency
            result = calllib(obj.libname, 'Lines_Get_Rg');
            obj.CheckForError();
        end
        function obj = set.Rg(obj, Value)
            calllib(obj.libname, 'Lines_Set_Rg', Value);
            obj.CheckForError();
        end

        function result = get.Rho(obj)
            % Earth Resistivity, m-ohms
            result = calllib(obj.libname, 'Lines_Get_Rho');
            obj.CheckForError();
        end
        function obj = set.Rho(obj, Value)
            calllib(obj.libname, 'Lines_Set_Rho', Value);
            obj.CheckForError();
        end

        function result = get.Rmatrix(obj)
            % Resistance matrix (full), ohms per unit length. Array of doubles.
            calllib(obj.libname, 'Lines_Get_Rmatrix_GR');
            obj.CheckForError();
            result = obj.apiutil.get_float64_gr_array();
        end
        function obj = set.Rmatrix(obj, Value)
            calllib(obj.libname, 'Lines_Set_Rmatrix', Value, numel(Value));
            obj.CheckForError();
        end

        function result = get.Spacing(obj)
            % Line spacing code
            result = calllib(obj.libname, 'Lines_Get_Spacing');
            obj.CheckForError();
        end
        function obj = set.Spacing(obj, Value)
            calllib(obj.libname, 'Lines_Set_Spacing', Value);
            obj.CheckForError();
        end

        function result = get.TotalCust(obj)
            % (read-only) Total Number of customers served from this line section.
            result = calllib(obj.libname, 'Lines_Get_TotalCust');
            obj.CheckForError();
        end

        function result = get.Units(obj)
            result = calllib(obj.libname, 'Lines_Get_Units');
            obj.CheckForError();
        end
        function obj = set.Units(obj, Value)
            calllib(obj.libname, 'Lines_Set_Units', Value);
            obj.CheckForError();
        end

        function result = get.X0(obj)
            % Zero Sequence reactance ohms per unit length.
            result = calllib(obj.libname, 'Lines_Get_X0');
            obj.CheckForError();
        end
        function obj = set.X0(obj, Value)
            calllib(obj.libname, 'Lines_Set_X0', Value);
            obj.CheckForError();
        end

        function result = get.X1(obj)
            % Positive Sequence reactance, ohms per unit length.
            result = calllib(obj.libname, 'Lines_Get_X1');
            obj.CheckForError();
        end
        function obj = set.X1(obj, Value)
            calllib(obj.libname, 'Lines_Set_X1', Value);
            obj.CheckForError();
        end

        function result = get.Xg(obj)
            % Earth return reactance value used to compute line impedances at power frequency
            result = calllib(obj.libname, 'Lines_Get_Xg');
            obj.CheckForError();
        end
        function obj = set.Xg(obj, Value)
            calllib(obj.libname, 'Lines_Set_Xg', Value);
            obj.CheckForError();
        end

        function result = get.Xmatrix(obj)
            calllib(obj.libname, 'Lines_Get_Xmatrix_GR');
            obj.CheckForError();
            result = obj.apiutil.get_float64_gr_array();
        end
        function obj = set.Xmatrix(obj, Value)
            calllib(obj.libname, 'Lines_Set_Xmatrix', Value, numel(Value));
            obj.CheckForError();
        end

        function result = get.Yprim(obj)
            % Yprimitive: Does Nothing at present on Put; Dangerous
            calllib(obj.libname, 'Lines_Get_Yprim_GR');
            obj.CheckForError();
            result = obj.apiutil.get_float64_gr_array();
        end
        function obj = set.Yprim(obj, Value)
            calllib(obj.libname, 'Lines_Set_Yprim', Value, numel(Value));
            obj.CheckForError();
        end
        ...
        function result = get.SeasonRating(obj)
            ...
            result = calllib(obj.libname, 'Lines_Get_SeasonRating');
            obj.CheckForError();
        end
        ...
        function result = get.IsSwitch(obj)
            ...
            result = (calllib(obj.libname, 'Lines_Get_IsSwitch') ~= 0);
            obj.CheckForError();
        end
        function obj = set.IsSwitch(obj, Value)
            calllib(obj.libname, 'Lines_Set_IsSwitch', Value);
            obj.CheckForError();
        end
    end
end