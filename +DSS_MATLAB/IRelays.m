classdef (CaseInsensitiveProperties) IRelays < DSS_MATLAB.Base
    ...
    methods
        ...
        function result = get.AllNames(obj)
            ...
            result = obj.apiutil.get_string_array('Relays_Get_AllNames');
        end

        function result = get.Count(obj)
            % Number of Relay objects
            result = calllib(obj.libname, 'Relays_Get_Count');
        end

        function result = get.First(obj)
            % Set first object of Relay; returns 0 if none.
            result = calllib(obj.libname, 'Relays_Get_First');
        end

        function result = get.Name(obj)
            % Get/sets the name of the current active Relay
            result = calllib(obj.libname, 'Relays_Get_Name');
        end
        function obj = set.Name(obj, Value)
            calllib(obj.libname, 'Relays_Set_Name', Value);
            obj.CheckForError();
        end

        function result = get.Next(obj)
            % Sets next Relay active; returns 0 if no more.
            result = calllib(obj.libname, 'Relays_Get_Next');
        end

        function result = get.idx(obj)
            % Get/set active Relay by index;  1..Count
            result = calllib(obj.libname, 'Relays_Get_idx');
        end
        function obj = set.idx(obj, Value)
            calllib(obj.libname, 'Relays_Set_idx', Value);
            obj.CheckForError();
        end
        ...
        function result = get.MonitoredObj(obj)
            ...
            result = calllib(obj.libname, 'Relays_Get_MonitoredObj');
            obj.CheckForError();
        end
        function obj = set.MonitoredObj(obj, Value)
            calllib(obj.libname, 'Relays_Set_MonitoredObj', Value);
            obj.CheckForError();
        end

        function result = get.MonitoredTerm(obj)
            % Number of terminal of monitored element that this Relay is monitoring.
            result = calllib(obj.libname, 'Relays_Get_MonitoredTerm');
            obj.CheckForError();
        end
        function obj = set.MonitoredTerm(obj, Value)
            calllib(obj.libname, 'Relays_Set_MonitoredTerm', Value);
            obj.CheckForError();
        end

        function result = get.SwitchedObj(obj)
            % Full name of element that will be switched when relay trips.
            result = calllib(obj.libname, 'Relays_Get_SwitchedObj');
            obj.CheckForError();
        end
        function obj = set.SwitchedObj(obj, Value)
            calllib(obj.libname, 'Relays_Set_SwitchedObj', Value);
            obj.CheckForError();
        end

        function result = get.SwitchedTerm(obj)
            % Terminal number of the switched object that will be opened when the relay trips.
            result = calllib(obj.libname, 'Relays_Get_SwitchedTerm');
            obj.CheckForError();
        end
        function obj = set.SwitchedTerm(obj, Value)
            calllib(obj.libname, 'Relays_Set_SwitchedTerm', Value);
            obj.CheckForError();
        end
    end
end